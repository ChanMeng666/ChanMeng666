// Render X (Twitter) inline media (e.g. the pinned-post product lineup) to PNG.
// The source HTML carries a `data-out` attribute naming its output file; this
// script screenshots that element at 2× so text stays crisp when X scales the
// 1200×675 media in-timeline.
//
// Usage:  node scripts/export-x-media.mjs
//         → writes x/media/pinned-products.png (2× = 2400×1350)
//
...
const dir = path.join(repoRoot, "x", "media");

const SOURCES = ["pinned-products.html"];
...
await context.close();
await browser.close();
console.log(`\nExported ${total} image(s) to x/media/`);
